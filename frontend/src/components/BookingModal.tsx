  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && editBooking) {
        await updateBooking({ id: editBooking.id, title, description, start_time: startISO, end_time: endISO, guests });
                ...
                {isReadOnly && editBooking ? (
                  <motion.div key="readonly" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    <div className="rounded-xl p-4 space-y-2"
                      ...
                      <p className="text-xs" style={{ color: "var(--text-muted)" }}>Автор: {tgUser?.name}</p>
